...
namespace Autodesk.APS.BIM360
{
    ...
    {
        string APSClientId { get; }
        string APSClientSecret { get; }

        string APSBimAccountId { get; }
        string BaseUrl { get; }
        string FilePath { get; }
    ...
    public class APSApi
    {
        ...
        public APSApi(Token token, ApplicationOptions options)
        {
            var baseUrl = string.IsNullOrWhiteSpace(options.BaseUrl) == false ? options.BaseUrl : "https://developer.api.autodesk.com";
            Client = new RestClient(new System.Uri(baseUrl));
        ...
        public RestResponse ExecuteRequest(RestRequest req)
        {
